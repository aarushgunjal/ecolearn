...
export function AdminReview() {
  ...
  const runAi = async (id: string) => { setMessage("Running AI review…"); const { data, error } = await supabase.functions.invoke("review-feedback", { body: { feedbackId: id } }); setMessage(error || data?.error ? `AI review failed: ${data?.error ?? error?.message}` : `AI review completed: ${data?.status}.`); void load(); };
  const setLabel = async (id: string, normalized_label: string) => { const { error } = await supabase.from("scan_feedback").update({ normalized_label, reviewer_kind: "human", review_status: "approved", reviewed_at: new Date().toISOString(), reviewed_by: user?.id, review_rationale: "Human reviewer assigned the final training label." }).eq("id", id); if (error) setMessage("Could not save the correct label."); else { setMessage("Correct label saved and approved."); void load(); } };
