...
serve(async (request) => {
  ...
  try {
    ...
    if (!classifierUrl)
      throw new Error("The classifier service is not configured.");
    const body = new FormData();
    body.append("file", file, file.name || "scan.jpg");
    // A multi-object scan runs the classifier for each detected crop; leave room
    // for a cold-starting inference Space without allowing an unbounded request.
    ...
    if (!response.ok)
      throw new Error("The classifier is temporarily unavailable.");
