...
serve(async (request) => {
  ...
  try {
    ...
    if (!classifierUrl) throw new Error("The classifier service is not configured.");
    ...
    if (!response.ok) throw new Error("The classifier is temporarily unavailable.");
    return Response.json(await response.json(), { headers: { ...corsHeaders, "Cache-Control": "no-store" } });
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : "Could not process image." }, { status: 400, headers: corsHeaders });
  }
});
