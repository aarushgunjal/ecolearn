...
type ScanResult = { item: string; recyclable: boolean; confidence: number; category: string; instructions: string; tips?: string[]; top_predictions?: { class: string; confidence: number }[]; dnrec?: { title: string; category: string; curbside: boolean; instructions: string; sourceName: string; sourceUrl: string } | null };
type DelawareGuidance = { title: string; category: string; curbside: boolean; instructions: string; sourceName: string; sourceUrl: string };
...
function ScanScreen({ user, onRecorded, onTools }: { user: User; onRecorded: () => Promise<void>; onTools: () => void }) {
  ...
  const choose = async (camera: boolean) => { const permission = camera ? await ImagePicker.requestCameraPermissionsAsync() : await ImagePicker.requestMediaLibraryPermissionsAsync(); if (!permission.granted) return Alert.alert("Permission needed", "Allow access to scan an item."); const picked = camera ? await ImagePicker.launchCameraAsync({ mediaTypes: ["images"], quality: 0.8 }) : await ImagePicker.launchImageLibraryAsync({ mediaTypes: ["images"], quality: 0.8 }); if (!picked.canceled && picked.assets[0]) { setPhoto(photoFromAsset(picked.assets[0])); setResult(null); setExplanation(null); setFeedback(null); } };
  const scan = async () => { if (!photo) return; setBusy(true); try { const form = new FormData(); form.append("file", { uri: photo.uri, type: photo.mimeType, name: photo.name } as unknown as Blob); const { data, error } = await supabase.functions.invoke("classify-scan", { body: form }); if (error) throw error; setResult(data as ScanResult); const { error: recordError } = await supabase.rpc("record_scan", { p_item_name: data.item, p_is_recyclable: Boolean(data.recyclable), p_confidence: Number(data.confidence ?? 0), p_category: data.category ?? "Unknown", p_instructions: data.instructions ?? "Check local guidance." }); if (recordError) console.warn(recordError.message); await onRecorded(); } catch (error) { Alert.alert("We couldn't scan that image", error instanceof Error ? error.message : "Try another clear photo."); } finally { setBusy(false); } };
  const explain = async () => { if (!photo || !result) return; setBusy(true); try { const base64 = await FileSystem.readAsStringAsync(photo.uri, { encoding: FileSystem.EncodingType.Base64 }); const { data, error } = await supabase.functions.invoke("explain-scan", { body: { image: `data:${photo.mimeType};base64,${base64}`, predictedLabel: result.item, predictedConfidence: result.confidence } }); if (error) throw error; setExplanation(data); } catch (error) { Alert.alert("AI explanation unavailable", "Please try again shortly."); } finally { setBusy(false); } };
