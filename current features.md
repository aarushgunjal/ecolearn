...
Recyclopedia data, guided learning, progress, and privacy-conscious scanning.
...
- Catalog queries combine the model's generic phrase, observed item, and likely
  material so branded labels such as Pepsi can resolve to official categories
  such as Aluminum Cans without a second model request.
...
- The scanner result-feedback collector is disabled; images sent for a visual
  lookup are not stored by EcoLearn or used for model training.
